...
(function($) {
    ...
    var sampleName = 'react',
        samplePath = 'samples/react/',
        zoomIn = false;
    ...
    function updateDepth(book, newPage) {
        ...
        if (newPage>3)
            $('.react-book .p2 .depth').css({
                width: depthWidth,
                left: 20 - depthWidth
            });
        else
            $('.react-book .p2 .depth').css({width: 0});
    
            depthWidth = Math.round(16*Math.min(1, (pages-page)*2/pages));
    
        if (newPage<pages-3)
            $('.react-book .p111 .depth').css({
                width: depthWidth,
                right: 20 - depthWidth
            });
        else
            $('.react-book .p111 .depth').css({width: 0});
    ...
    function loadPage(page) {
       ...
       // if (page==3)
            $.ajax({url: samplePath + 'pages/page' + page + '.html?'+Math.random()}).
                    ...
                    $('.react-book .p' + page).html(pageHtml);
                });
    ...
    function loadFlipbook(flipbook) {
        ...
        flipbook.turn({
            ...
            turning: function(e, page, view) {
                ...
                if (page>=2)
                    $('.react-book .p2').addClass('fixed');
                else
                    $('.react-book .p2').removeClass('fixed');
    
                if (page<book.turn('pages'))
                    $('.react-book .p111').addClass('fixed');
                else
                    $('.react-book .p111').removeClass('fixed');
    ...
    bookshelf.loadSample(sampleName, function(action) {
        ...
        sample.shareText = 'A book about Steve Jobs in react using the new turn.js via @turnjs';
        ...
        if (!sample.flipbook) {
            ...
            var bookClass = (Modernizr.csstransforms) ?
                'react-book-transform react-book' :
                'react-book';
