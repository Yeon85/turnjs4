...
(function($) {
    ...
    var sampleName = 'three',
        samplePath = 'samples/three/',
        zoomIn = false;
    ...
    function updateDepth(book, newPage) {
        ...
        if (newPage>3)
            $('.three-book .p2 .depth').css({
                width: depthWidth,
                left: 20 - depthWidth
            });
        else
            $('.three-book .p2 .depth').css({width: 0});
    
            depthWidth = Math.round(16*Math.min(1, (pages-page)*2/pages));
    
        if (newPage<pages-3)
            $('.three-book .p111 .depth').css({
                width: depthWidth,
                right: 20 - depthWidth
            });
        else
            $('.three-book .p111 .depth').css({width: 0});
    ...
    function loadPage(page) {
       ...
       // if (page==3)
            $.ajax({url: samplePath + 'pages/page' + page + '.html?'+Math.random()}).
                    ...
                    $('.three-book .p' + page).html(pageHtml);
                });
    ...
    function loadFlipbook(flipbook) {
        ...
        flipbook.turn({
            ...
            turning: function(e, page, view) {
                ...
                if (page>=2)
                    $('.three-book .p2').addClass('fixed');
                else
                    $('.three-book .p2').removeClass('fixed');
    
                if (page<book.turn('pages'))
                    $('.three-book .p111').addClass('fixed');
                else
                    $('.three-book .p111').removeClass('fixed');
    ...
    bookshelf.loadSample(sampleName, function(action) {
        ...
        sample.shareText = 'A book about Steve Jobs in three using the new turn.js via @turnjs';
        ...
        if (!sample.flipbook) {
            ...
            var bookClass = (Modernizr.csstransforms) ?
                'three-book-transform three-book' :
                'three-book';
